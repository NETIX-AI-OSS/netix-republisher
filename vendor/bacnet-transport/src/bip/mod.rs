...
pub struct BipTransport {
    ...
    /// Destination UDP port for broadcasts. Decoupled from the local bind port so
    /// that an ephemeral bind (`port == 0`) still broadcasts to the standard
    /// BACnet/IP port instead of the OS-assigned local port.
    broadcast_port: u16,
    local_mac: [u8; 6],
...
impl BipTransport {
    ...
    /// - `port`: Local UDP bind port (`0` for an OS-assigned ephemeral port;
    ///   broadcasts are then sent to the standard 47808 / 0xBAC0)
    ...
    pub fn new(interface: Ipv4Addr, port: u16, broadcast_address: Ipv4Addr) -> Self {
        Self {
            ...
            port,
            broadcast_address,
            broadcast_port: if port == 0 { DEFAULT_BACNET_PORT } else { port },
            local_mac: [0; 6],
            socket: None,
            recv_task: None,
            bbmd_config: None,
...
impl TransportPort for BipTransport {
    ...
    async fn send_broadcast(&self, npdu: &[u8]) -> Result<(), Error> {
        ...
        let dest = SocketAddrV4::new(self.broadcast_address, self.broadcast_port);
