...
#[test]
fn ephemeral_bind_broadcasts_to_default_bacnet_port() {
    let transport = BipTransport::new(Ipv4Addr::LOCALHOST, 0, Ipv4Addr::BROADCAST);
    assert_eq!(transport.broadcast_port, DEFAULT_BACNET_PORT);
}

#[test]
fn explicit_bind_port_is_used_as_broadcast_destination() {
    let transport = BipTransport::new(Ipv4Addr::LOCALHOST, 47812, Ipv4Addr::BROADCAST);
    assert_eq!(transport.broadcast_port, 47812);
}

#[tokio::test]
async fn ephemeral_bind_keeps_broadcast_port_after_start() {
    // Regression: broadcast destination used to follow the OS-assigned local
    // port after start(), so Who-Is broadcasts from an ephemeral bind never
    // reached devices listening on 47808.
    let mut transport = BipTransport::new(Ipv4Addr::LOCALHOST, 0, Ipv4Addr::BROADCAST);
    let _rx = transport.start().await.unwrap();
    assert_ne!(transport.port, 0, "start() should record the local port");
    assert_eq!(transport.broadcast_port, DEFAULT_BACNET_PORT);
